...
def one_eval(scope):
    images_test, labels_test = ipNet.inputs(eval_data=True)
    logits_test = ipNet.inference_switch(images_test, FLAGS.network_architecture)
...
def tower_loss(scope):
  ...
  images, labels = ipNet.distorted_inputs()

  # Build inference Graph.
  logits = ipNet.inference_switch(images, FLAGS.network_architecture)

  # Build the portion of the Graph calculating the losses. Note that we will
  # assemble the total_loss using a custom function below.
  _ = ipNet.loss(logits, labels)
  ...
  for l in losses + [total_loss]:
    ...
    loss_name = re.sub('%s_[0-9]*/' % ipNet.TOWER_NAME, '', l.op.name)
...
def train():
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    num_batches_per_epoch = (ipNet.NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN / FLAGS.batch_size)
    decay_steps = int(num_batches_per_epoch * ipNet.NUM_EPOCHS_PER_DECAY)
    # decay_steps = 1500

    # Decay the learning rate exponentially based on the number of steps.
    lr = tf.train.exponential_decay(ipNet.INITIAL_LEARNING_RATE,
                                    global_step,
                                    decay_steps,
    ...
    with tf.variable_scope(tf.get_variable_scope()):
        for i in xrange(FLAGS.num_gpus):
          with tf.device('/gpu:%d' % i):
            with tf.name_scope('%s_%d' % (ipNet.TOWER_NAME, i)) as scope:
    ...
    variable_averages = tf.train.ExponentialMovingAverage(ipNet.MOVING_AVERAGE_DECAY, global_step)
...
def main_justTheOne(argv=None):  # pylint: disable=unused-argument
    ...
    idx = 0
    precision, cm = ipNet_eval.evaluate()
