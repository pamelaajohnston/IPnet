...
tf.app.flags.DEFINE_integer('num_examples', 1650, """Number of examples to run.""")
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            while step < num_iter and not coord.should_stop():
                ...
                batchConfusionMatrix = batchConfusionMatrix.reshape((ipNet_input.NUM_CLASSES, ipNet_input.NUM_CLASSES))
                if step == 0:
...
def evaluate_orig():
  ...
  with tf.Graph().as_default() as g:
    ...
    images, labels = ipNet.inputs(eval_data=eval_data)

    # Build a Graph that computes the logits predictions from the
    # inference model.
    logits = ipNet.inference(images)

    # Calculate predictions.
    top_k_op = tf.nn.in_top_k(logits, labels, 1)

    # Restore the moving average version of the learned variables for eval.
    variable_averages = tf.train.ExponentialMovingAverage(ipNet.MOVING_AVERAGE_DECAY)
...
def evaluate(returnConfusionMatrix=True):
    ...
    with tf.Graph().as_default() as g:
        ...
        eval_data = FLAGS.eval_data == 'test'
        images, labels = ipNet.inputs(eval_data=True)
